...
function LoginPageContent() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      try {
        ...
        // Firebase sometimes throws IndexedDB "database closing" errors even when auth succeeded
      ...
      setError(firebaseAuthErrorMessage(err, 'Google sign-in failed'));
  ...
  const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      const { data } = await api.post('/auth/login', { email, password, storeId: store?.id });
      setAuth(data.user, data.accessToken, data.refreshToken);
      router.push(next);
